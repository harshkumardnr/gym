...
const Member = () => {
    ...
    const handlePrev = () => {
        if (currentPage !== 1) {
            ...
            var from = (currPage - 1) * 30;
            var to = (currPage * 30);
            setStartFrom(from);
            setEndTo(to);
            let skipValue = skip - 30;
            setSkip(skipValue);
            fetchData(skipValue, 30);
        }
    }
    const handleNext = () => {
    ...
    return (
            ...
            <div className='mt-2 text-xl flex justify-between text-slate-900'>
            ...
            <div className='bg-slate-100 p-5 rounded-lg grid gap-2 grid-cols-5 overflow-x-auto h-[75%]'>
                {
                    data.map((item, index) => {
                        return (
